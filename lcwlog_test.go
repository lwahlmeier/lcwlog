...
func TestArgPosTest(t *testing.T) {
	resetLogger()
	slc := GetLoggerConfig()
	slc.EnableTimeLogging(false)
	slc.EnableLevelLogging(false)
	slc.RemoveLogFile("STDOUT")
	sl := GetLogger()
	slc.SetLevel(DebugLevel)
	tmpfile, err := ioutil.TempFile("", ".TESTLOG")
	defer os.Remove(tmpfile.Name())
	check(err)
	slc.AddLogFile(tmpfile.Name(), sl.GetLogLevel())
	sl.Warn("Warn {1} {0}", "Message", "test")
...
func BenchmarkNoSyncArgPosFormat(b *testing.B) {
	b.ReportAllocs()
	resetLogger()
	sl := GetLogger()
	slc := GetLoggerConfig()
